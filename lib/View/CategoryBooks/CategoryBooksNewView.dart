...
class CategoryBooksNewView extends StatefulWidget {
  String category;
  CatalogData catalog;
  CategoryBooksNewView({required this.category,required this.catalog});

  @override
  State<CategoryBooksNewView> createState() => _CategoryBooksNewViewState();
}

class _CategoryBooksNewViewState extends State<CategoryBooksNewView> {
  int isDark = 0;
  ...
  void initState() {
    ...
    categoryController!.addPageRequestListener((pageKey) {
      context.read<CategoryListCubit>().getCategoryBooksCatalog(category: widget.category, href: widget.catalog.href ?? "", query: "");
    });
    initStateData();
    super.initState();
  }
